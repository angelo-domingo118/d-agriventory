...
<div x-data="{ 
    ...tableResizer('contracts_widths', { contract_number: 300, supplier: 250, items: 150, date: 180, actions: 100 }),
    ...tableSettings('contracts_settings')
}">
                             ...
                             @forelse($this->contracts as $contract)
                                <tr wire:key="contract-{{ $contract->id }}" class="hover:bg-stone-50 dark:hover:bg-stone-800/50">
                                    ...
                                    <td class="whitespace-nowrap {{ $densityClasses['table_cell'] }} text-right {{ $densityClasses['text_base'] }} font-medium">
                                        <button wire:click="editContract({{ $contract->id }})" wire:loading.attr="disabled" wire:target="editContract({{ $contract->id }})" class="inline-flex items-center rounded-md border border-stone-300 bg-white px-2.5 py-1.5 {{ $densityClasses['text_base'] }} font-semibold text-stone-900 shadow-sm hover:bg-stone-50 disabled:opacity-50 disabled:cursor-not-allowed dark:border-stone-700 dark:bg-stone-800 dark:text-stone-200 dark:hover:bg-stone-700/50">
                                           <x-flux::icon.pencil class="mr-1.5 h-4 w-4" wire:loading.remove wire:target="editContract({{ $contract->id }})" />
                                           <x-flux::icon.rotate-cw class="mr-1.5 h-4 w-4 animate-spin" wire:loading wire:target="editContract({{ $contract->id }})" />
                                           <span wire:loading.remove wire:target="editContract({{ $contract->id }})">Edit</span>
                                           <span wire:loading wire:target="editContract({{ $contract->id }})">Loading...</span>
                                        </button>
                                    </td>
                                </tr>
                            @empty
    ...
    <x-admin.modal-form-wrapper name="create-contract" maxWidth="lg">
        <livewire:admin.data.suppliers-and-contracts.contracts.create />
    </x-admin.modal-form-wrapper>

    <!-- Edit Contract Modal -->
    @if($editingContract)
        <x-admin.modal-form-wrapper name="edit-contract" maxWidth="lg">
