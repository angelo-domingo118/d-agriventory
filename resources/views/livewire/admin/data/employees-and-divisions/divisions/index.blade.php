...
new #[Layout('components.layouts.app')] class extends Component {
    ...
    public function mount()
    {
        ...
        $this->density = session('divisions_density', 'spacious');
    }

    public function setDensity(string $density): void
    {
        $this->density = $density;
        session(['divisions_density' => $density]);
    }
                ...
                <tbody class="divide-y divide-stone-200 bg-white dark:divide-stone-800 dark:bg-stone-900">
                    @forelse($divisions as $division)
                        <tr wire:key="division-{{ $division->id }}" class="hover:bg-stone-50 dark:hover:bg-stone-800/50">
                            <td class="whitespace-nowrap {{ $densityClasses['table_cell'] }} {{ $densityClasses['text_base'] }} font-medium text-stone-900 dark:text-stone-100">{{ $division->name }}</td>
                            <td class="whitespace-nowrap {{ $densityClasses['table_cell'] }} {{ $densityClasses['text_base'] }} text-stone-500 dark:text-stone-400">{{ $division->code }}</td>
