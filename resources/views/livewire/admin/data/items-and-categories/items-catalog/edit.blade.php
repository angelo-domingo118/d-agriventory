...
new #[Layout('components.layouts.app')] class extends Component {
    ...
    {
        $this->validate([
            'item.name' => ['required', 'string', 'max:255', Rule::unique('items_catalog', 'name')->ignore($this->item->id)],
            'item.code' => ['required', 'string', 'max:50', Rule::unique('items_catalog', 'code')->ignore($this->item->id)],
        ...
        try {
            ...
            $this->redirectRoute('admin.data.items-and-categories.index', ['currentTab' => 'items']);
        } catch (\Exception $e) {
    ...
    public function delete(): void
    {
        try {
            if (
                $this->item->contractItems()->exists() ||
                $this->item->icsItemBatches()->exists() ||
                $this->item->parItemBatches()->exists() ||
                $this->item->idrItemBatches()->exists() ||
                $this->item->components()->exists() ||
                $this->item->consumableItems()->exists()
            ) {
            ...
            <div class="space-y-6 rounded-lg border border-stone-200 bg-white p-6 shadow-sm dark:border-stone-700 dark:bg-stone-800">
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    ...
                    <div class="sm:col-span-2">
                        <flux:textarea wire:model="item.description" label="Description" />
                    </div>
