...
new #[Layout('components.layouts.app')] class extends Component {
    ...
    public ?int $items_catalog_id = null; // Added for item catalog support
    public ?string $item_specification_id = null; // Can be "new" or an integer ID
    ...
    public int $quantity_per_batch = 1; // Added for batch quantity control
    ...
    public bool $creating_new_assigned_employee = false;
    ...
    public bool $creating_new_approving_employee = false;
    ...
    public bool $creating_new_received_by_employee = false;
    ...
    public bool $creating_new_received_from_employee = false;
    ...
    {
        // Find the highest numeric IDR number and its batch count
        $lastIdr = IdrNumber::orderByRaw('CAST(number AS UNSIGNED) DESC')->first();
        
        if ($lastIdr) {
            // Count the number of batches for the last IDR number
            $batchCount = IdrItemBatch::where('idr_number_id', $lastIdr->id)->count();
            ...
            // Calculated based on previous IDR number + batch count
            $this->number = (string)(((int) $lastIdr->number) + $batchCount);
        } else {
    ...
    public function selectSpecification($specData): void
    {
        ...
        if ($specData['type'] === 'existing') {
            ...
            // Focus on unit cost if no pricing was found, otherwise go to assigned employee
    ...
    public function searchEmployees($query, $type): void
    {
        ...
        $exactExists = collect($suggestions)->contains(function ($employee) use ($query) {
    ...
    public function selectAssignedEmployee($employeeData): void
    {
        ...
        if ($employeeData['type'] === 'existing') {
            ...
            $this->creating_new_assigned_employee = false;
        } elseif ($employeeData['type'] === 'new') {
            $this->assigned_employee_id = null;
            $this->assigned_employee_search = $employeeData['name'];
            $this->selected_assigned_employee_name = $employeeData['name'] . ' (new)';
            $this->creating_new_assigned_employee = true;
        }
    ...
    public function selectApprovingEmployee($employeeData): void
    {
        ...
        if ($employeeData['type'] === 'existing') {
            ...
            $this->creating_new_approving_employee = false;
        } elseif ($employeeData['type'] === 'new') {
            $this->approving_employee_id = null;
            $this->approving_employee_search = $employeeData['name'];
            $this->selected_approving_employee_name = $employeeData['name'] . ' (new)';
            $this->creating_new_approving_employee = true;
        }
    ...
    public function selectReceivedByEmployee($employeeData): void
    {
        ...
        if ($employeeData['type'] === 'existing') {
            ...
            $this->creating_new_received_by_employee = false;
        } elseif ($employeeData['type'] === 'new') {
            $this->received_by_id = null;
            $this->received_by_search = $employeeData['name'];
            $this->selected_received_by_name = $employeeData['name'] . ' (new)';
            $this->creating_new_received_by_employee = true;
        }
    ...
    public function selectReceivedFromEmployee($employeeData): void
    {
        ...
        if ($employeeData['type'] === 'existing') {
            ...
            $this->creating_new_received_from_employee = false;
        } elseif ($employeeData['type'] === 'new') {
            $this->received_from_id = null;
            $this->received_from_search = $employeeData['name'];
            $this->selected_received_from_name = $employeeData['name'] . ' (new)';
            $this->creating_new_received_from_employee = true;
        }
    ...
    private function updateItemType(): void
    {
        ...
        // Format unit price and set type based on value
    ...
    <div class="mt-6">
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-2 xl:grid-cols-3">
                                ...
                                @if ($isParItem)
                                    <div class="mt-4 flex w-full items-center rounded-lg bg-red-50 p-3 text-sm text-red-700 dark:bg-red-800/20 dark:text-red-400" role="alert">
                                        ...
                                        <span class="font-medium">
                                            The unit cost of ₱{{ number_format($this->unit_price, 2) }} for this item requires it to be recorded on a Property Accountability Receipt (PAR), not an IDR.
                                        </span>
                                    </div>
                                @endif
                            </div>
                ...
                <div class="overflow-hidden rounded-lg border border-stone-200 bg-white shadow-sm dark:border-stone-700 dark:bg-stone-800">
                    ...
                    <div class="space-y-4 p-4">
                        <div>
                            ...
                            @enderror
                            @if ($creating_new_assigned_employee)
                                <div class="mt-2 flex items-center rounded-lg bg-green-50 p-2 text-sm text-green-700 dark:bg-green-800/20 dark:text-green-400" role="alert">
                                    <svg class="mr-2 h-4 w-4 flex-shrink-0" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                                        <path d="M10 .5a9.5 9.5 0 1 0 9.5 9.5A9.51 9.51 0 0 0 10 .5ZM9.5 4a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3ZM12 15H8a1 1 0 0 1 0-2h1v-3H8a1 1 0 0 1 0-2h2a1 1 0 0 1 1 1v4h1a1 1 0 0 1 0 2Z" />
                                    </svg>
                        ...
                        <div>
                            ...
                            @enderror
                            @if ($creating_new_approving_employee)
                                <div class="mt-2 flex items-center rounded-lg bg-green-50 p-2 text-sm text-green-700 dark:bg-green-800/20 dark:text-green-400" role="alert">
                                    <svg class="mr-2 h-4 w-4 flex-shrink-0" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                                        <path d="M10 .5a9.5 9.5 0 1 0 9.5 9.5A9.51 9.51 0 0 0 10 .5ZM9.5 4a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3ZM12 15H8a1 1 0 0 1 0-2h1v-3H8a1 1 0 0 1 0-2h2a1 1 0 0 1 1 1v4h1a1 1 0 0 1 0 2Z" />
                                    </svg>
                        ...
                        <div>
                            ...
                            @enderror
                            @if ($creating_new_received_by_employee)
                                <div class="mt-2 flex items-center rounded-lg bg-green-50 p-2 text-sm text-green-700 dark:bg-green-800/20 dark:text-green-400" role="alert">
                                    <svg class="mr-2 h-4 w-4 flex-shrink-0" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                                        <path d="M10 .5a9.5 9.5 0 1 0 9.5 9.5A9.51 9.51 0 0 0 10 .5ZM9.5 4a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3ZM12 15H8a1 1 0 0 1 0-2h1v-3H8a1 1 0 0 1 0-2h2a1 1 0 0 1 1 1v4h1a1 1 0 0 1 0 2Z" />
                                    </svg>
                        ...
                        <div>
                            ...
                            @enderror
                            @if ($creating_new_received_from_employee)
                                <div class="mt-2 flex items-center rounded-lg bg-green-50 p-2 text-sm text-green-700 dark:bg-green-800/20 dark:text-green-400" role="alert">
                                    <svg class="mr-2 h-4 w-4 flex-shrink-0" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                                        <path d="M10 .5a9.5 9.5 0 1 0 9.5 9.5A9.51 9.51 0 0 0 10 .5ZM9.5 4a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3ZM12 15H8a1 1 0 0 1 0-2h1v-3H8a1 1 0 0 1 0-2h2a1 1 0 0 1 1 1v4h1a1 1 0 0 1 0 2Z" />
                                    </svg>
                        ...
                        <div class="grid grid-cols-1 gap-x-6 sm:grid-cols-2">
                            ...
                            <div>
                                <flux:input
                                    ...
                                    @input="formatDate($event)"
                                    @keydown.enter.prevent=""
                                    @keydown.tab="if (!event.shiftKey) { event.preventDefault(); $wire.dispatch('focus-date-idr'); }"
                                />
            ...
            <div class="space-y-6 lg:col-span-2 xl:col-span-1">
                ...
                <div class="overflow-hidden rounded-lg border border-stone-200 bg-white shadow-sm dark:border-stone-700 dark:bg-stone-800" x-data="{
                    autoSerial: false,
                    get showQuantityPerBatch() {
                        return !$wire.isParItem && ($wire.selected_item_name && $wire.selected_item_name !== '');
                    }
                }">
                    <div class="border-b border-stone-200 px-4 py-3 dark:border-stone-700">
                    ...
                    <div class="p-4">
                        <div class="space-y-4">
                            <!-- Number of Batches and Quantity per Batch Controls -->
                            <div class="grid grid-cols-1 gap-x-6 gap-y-4 sm:grid-cols-2">
                                <div>
                                    <x-quantity-input 
                                        id="quantity"
                                        ...
                                        required
                                        class="w-full" 
                                        @keydown.tab="if (!event.shiftKey) { event.preventDefault(); $wire.dispatch('focus-serial-0'); }"
                                    />
                                    <x-input-error for="quantity" class="mt-2" />
                                </div>
                                <div x-show="showQuantityPerBatch">
                                    <x-quantity-input 
                                        ...
                                        class="w-full" 
                                    />
                                    <x-input-error for="quantity_per_batch" class="mt-2" />
                                </div>
                            </div>

                            <!-- Auto-populate Serial Number Checkbox -->
                            <div x-show="showQuantityPerBatch" class="flex items-center space-x-2">
                                <flux:checkbox 
                                    id="auto_serial" 
                                    x-model="autoSerial" 
                                    label="Auto-populate serial number with prefix" 
                                />
                                <p class="text-sm text-stone-500 dark:text-stone-400">This will generate serial numbers like "Item-001", "Item-002", etc.</p>
                            </div>
                            ...
                            <div class="space-y-4">
                                    ...
                                    <div wire:key="batch-{{ $batchIndex }}" class="rounded-lg border border-stone-300 bg-white p-0 dark:border-stone-600 dark:bg-stone-800/50">
                                        <div class="flex items-center justify-between p-3 bg-stone-50 dark:bg-stone-700/50 rounded-t-lg border-b border-stone-200 dark:border-stone-700">
                                            <h4 class="font-semibold text-stone-800 dark:text-stone-200 flex items-center space-x-2">
                                                ...
                                                @if ($quantity_per_batch > 1 && !$isParItem)
                                                    <span class="text-sm text-stone-500 dark:text-stone-400 font-normal">({{ $quantity_per_batch }} items)</span>
                                                @endif
                                            </h4>
                                        ...
                                        <div class="p-3">
                                            <flux:input 
                                                id="serial-{{ $batchIndex }}"
                                                wire:model="batches.{{ $batchIndex }}.identification_data" 
                                                label="Serial Number/Asset Tag" 
                                                x-bind:placeholder="autoSerial ? 'Will be auto-generated' : 'Enter serial number, asset tag or other identifying info'"
                                                x-bind:disabled="autoSerial" 
                                                x-bind:value="autoSerial ? 'AUTO-' + String({{ $batchIndex + 1 }}).padStart(3, '0') : null"
                                                x-init="if (autoSerial) { $wire.set('batches.{{ $batchIndex }}.identification_data', 'AUTO-' + String({{ $batchIndex + 1 }}).padStart(3, '0')); }"
                                                @keydown.tab="if (!event.shiftKey && {{ $batchIndex + 1 }} < $wire.quantity) { event.preventDefault(); $wire.dispatch('focus-serial-{{ $batchIndex + 1 }}'); }"
                                            />
                                        </div>

                                        <!-- Desktop Computer Components Section -->
                                        @if ($isDesktopComputer)
                                            <div class="border-t border-stone-200 bg-stone-25 p-3 dark:border-stone-700 dark:bg-stone-900/25">
                                                <h5 class="mb-3 text-sm font-medium text-stone-700 dark:text-stone-300">Desktop Computer Components</h5>
                                                
                                                @if (isset($batch['components']) && is_array($batch['components']))
                                                    @foreach ($batch['components'] as $componentIndex => $component)
                                                        <div wire:key="component-{{ $batchIndex }}-{{ $componentIndex }}" class="mb-3 grid grid-cols-1 gap-x-4 gap-y-2 sm:grid-cols-3">
                                                            <div>
                                                                <flux:input 
                                                                    wire:model="batches.{{ $batchIndex }}.components.{{ $componentIndex }}.component_type" 
                                                            ...
                                                            <div>
                                                                <flux:input 
                                                                    wire:model="batches.{{ $batchIndex }}.components.{{ $componentIndex }}.brand" 
                                                            ...
                                                            <div class="flex items-end space-x-2">
                                                                <div class="flex-1">
                                                                    <flux:input 
                                                                        wire:model="batches.{{ $batchIndex }}.components.{{ $componentIndex }}.model" 
                                                                        placeholder="e.g., P2414H" 
                                                                        label="Model" 
                                                                        size="sm" 
                                                                    />
                                                                </div>
                                                                @if (count($batch['components']) > 1)
                                                                    <flux:button 
                                                                        ...
                                                                        variant="danger" 
                                                                        size="sm" 
                                                                        wire:click.prevent="removeComponent({{ $batchIndex }}, {{ $componentIndex }})"
                                                                        class="mb-1"
                                                                    >
                                                                        <x-flux::icon.trash class="h-3 w-3" />
                                                                    </flux:button>
                                                                @endif
                                                            </div>
                                                            <div class="sm:col-span-3">
                                                                <flux:input 
                                                                    wire:model="batches.{{ $batchIndex }}.components.{{ $componentIndex }}.serial_number" 
                                                ...
                                                <div class="mt-2">
                                                    <flux:button 
                                                        ...
                                                        variant="outline" 
                                                        size="sm" 
                                                        wire:click.prevent="addComponent({{ $batchIndex }})"
                                                        class="w-full"
                                                    >
                                                        <x-flux::icon.plus class="mr-1 h-3 w-3" />
                                                        Add Component
                                                    </flux:button>
                                                </div>
                                            </div>
                                ...
                                <div class="mt-4 text-center">
                                    <flux:button 
                                        type="button" 
                                        variant="outline" 
                                        wire:click.prevent="$set('quantity', {{ $quantity + 1 }})" 
                                        class="w-full"
                                    >
