...
@php
    $secondaryCount = $category ? $category->secondaryCategories()->count() : 0;
    $itemsCount = $category ? $category->secondaryCategories()->withCount('items')->get()->sum('items_count') : 0;
    $hasAssociatedData = $secondaryCount > 0 || $itemsCount > 0;
@endphp

<x-admin.modal-form-wrapper :name="$name" maxWidth="lg">
    <div class="space-y-6">
        <!-- Warning Header -->
            ...
            <div class="mt-4">
                ...
                <flux:text class="mt-2 text-stone-600 dark:text-stone-400">
                    @if($hasAssociatedData)
                        This category has associated data that will be affected.
                    @else
                ...
                <div class="ml-3">
                    ...
                    @if($hasAssociatedData)
                        ...
                        <ul class="mt-2 text-sm text-red-700 dark:text-red-300 list-disc list-inside">
                            @if($secondaryCount > 0)
                                <li><strong>{{ $secondaryCount }}</strong> secondary {{ Str::plural('category', $secondaryCount) }}</li>
                            @endif
                            @if($itemsCount > 0)
                                <li><strong>{{ $itemsCount }}</strong> catalog {{ Str::plural('item', $itemsCount) }}</li>
                            @endif
                        </ul>
                    ...
                    @else
                        <p class="mt-1 text-sm text-red-700 dark:text-red-300">
                            This category has no associated secondary categories or items.
                        </p>
                    @endif
                </div>
            </div>
